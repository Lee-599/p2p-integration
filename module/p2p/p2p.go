...
// P2PListenerInfoOutput  p2p监听或映射信息
...
// Listen 将本地端口映射到p2p网络
func (c *P2pClient) Listen(targetAddr string) error {
...
func (c *P2pClient) Forward(port int, peerId string) error {
	...
	fmt.Println("远程节点" + peerId + ",forward 到" + listenOpt + "成功")
	return err
}
...
func (c *P2pClient) Close(target string) (int, error) {
	...
	match := func(listener ipfsp2p.Listener) bool {
		...
		if !targetAddress.Equal(listener.TargetAddress()) && !targetAddress.Equal(listener.ListenAddress()) {
			return false
		}
		return true
	}
	...
	for _, s := range c.P2P.Streams.Streams {
		...
		if s.OriginAddr.Equal(targetAddress) || s.TargetAddr.Equal(targetAddress) {
...
func (c *P2pClient) isLocalListening(target string) (bool, string) {
	listenAddress, err := ma.NewMultiaddr(target)
	if err != nil {
		return false, ""
	}
	for _, l := range c.P2P.ListenersP2P.Listeners {
		if l.ListenAddress().Equal(listenAddress) {
			port, err := l.TargetAddress().ValueForProtocol(6)
			if err != nil {
				return false, ""
			}
			return true, port
...
// Destroy 销毁关闭 p2p客户端，包括下属的所有监听，流对象
